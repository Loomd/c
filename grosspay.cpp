...
void getHoursWorked(double& hoursWorked) {
    cout << "\nEnter Hours worked (enter 0 if you wish to exit): ";
    while(!(cin >> hoursWorked) || hoursWorked < 0 ){
        cout << "Error, value entered was less than 0. Enter a value greater then 0: ";
        cin.clear();
        cin.ignore(100,'\n');
        continue;
    }
...
double calculateGrossPay(double& grossPay, double hoursWorked, double hourlyPay){
    grossPay = hoursWorked * hourlyPay;
    return grossPay;
}
//method should use a single-sided decision to add(+) over-time-pay equal(==) to half(.5) of the hourly pay-rate multiplied(*) by hours-worked over(/) 40
...
int main(){
    ...
    calculateGrossPay(grossPay, hoursWorked, hourlyPay);
